...
export function preparePackageJson(options: JsonObject & ServerlessDeployBuilderOptions,
  context: BuilderContext, 
  stats: any, 
  resolverName: string,
  tsconfig?: string): Observable<BuilderOutput> {
  ...
  return from(getProjectRoot(context)).pipe(
    ...
    concatMap((prodModules: string[]) => {
      ...
      if (packager('yarn')) {
        ...
        const result = packagerInstance.generateLockFile(dirname(packageJsonPath));
        if (result.error) {
          context.logger.error('ERROR: generating lock file!');
          return of({ success: false, error: result.error.toString() });
        }
        writeToFile(join(options.package, packagerInstance.lockfileName), result.stdout.toString());
      }
      ...
      const getDependenciesResult = packagerInstance.getProdDependencies(dirname(packageJsonPath), 1, 4);
    ...
    concatMap((prodModules: string[]) => {
      ...
      const packageInstallResult = packagerInstance.install(dirname(packageJsonPath), { ignoreScripts: true });
...
function resolverFactory(resolverName: string, context: BuilderContext): DependencyResolver {
