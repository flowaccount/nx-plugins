import { AwsCdkClient, awsCdkFactory, cdkDeployFlags, cdkFlags, getDeployOptions } from '@flowaccount/aws-cdk-core'
...
export default async function runSynthExecutor(
  ...
  awscdkClient.deploy(`${options.stackName}-${stackSuffix}`, cdkOption, deployOption);
  return {
    success: true,
  };
}
