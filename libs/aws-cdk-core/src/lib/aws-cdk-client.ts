...
import { cdkDeployOptions, cdkDestroyOptions, cdkDiffOptions, cdkOptions, cdkSynthOptions } from './types';
...
export class AwsCdkClient {
  ...
  private cdkCommandRunner(command: string, stackName: string, cdkParameters: cdkOptions, paramString: string) : Buffer  {
    let cmd = `${this.cliCommand.command} ${command} ${stackName}`;
    const cdkParamString = cdkParameters ? getParameterString(cdkParameters) : '';
  ...
  synth( stackName: string, cdkParameters: cdkOptions, parameters: cdkSynthOptions) : Buffer {
    const paramString = parameters ? getParameterString(parameters) : '';
    return this.cdkCommandRunner('synth', stackName, cdkParameters, paramString)
  }

  diff(stackName: string, cdkParameters: cdkOptions, parameters: cdkDiffOptions): Buffer {
    const paramString = parameters ? getParameterString(parameters) : '';
    return this.cdkCommandRunner('diff', stackName, cdkParameters, paramString)
  }

  deploy(stackName: string, cdkParameters: cdkOptions, parameters: cdkDeployOptions): Buffer {
    const paramString = parameters ? getParameterString(parameters) : '';
    return this.cdkCommandRunner('deploy', stackName, cdkParameters, paramString)
  }

  destroy(stackName: string, cdkParameters: cdkOptions, parameters: cdkDestroyOptions): Buffer {
    const paramString = parameters ? getParameterString(parameters) : '';
    return this.cdkCommandRunner('destroy', stackName, cdkParameters, paramString)
  }
