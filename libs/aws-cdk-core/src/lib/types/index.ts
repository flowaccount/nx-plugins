...
, 'progress'
, 'no-execute'
, 'change-set-name'] as const;
export type cdkSynthFlags = (typeof cdkSynthFlagsArray)[number];

export function iscdkSynthFlag(flag: cdkSynthFlags | string): flag is cdkSynthFlags {
...
const cdkDeployFlagsArray = [ 
  ...
  , 'previous-parameters'
  , 'progress'
  , 'no-rollback'] as const;

export type cdkDeployFlags = (typeof cdkDeployFlagsArray)[number];
export function iscdkDeployFlag(flag: cdkDeployFlags | string): flag is cdkDeployFlags {
  return cdkDeployFlagsArray.includes((flag as cdkDeployFlags));
}

export type cdkDestroyFlags = 
| 'exclusively'
| 'force'
| 'all'
 
export type cdkBootstrapFlags = 
| 'profile'
| 'app'
| 'template'
