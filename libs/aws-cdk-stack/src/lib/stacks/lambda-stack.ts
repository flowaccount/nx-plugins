...
export class TypescriptLambdaStack extends Stack {
  ...
  constructor(scope: cdk.Construct, id: string, _props: LambdaStackProperties) {
    ...
    _props.functions.forEach(f => {
      ...
      logger.info(`Initiating Lambda Function ${f.functionName}:${rootPath}/${f.srcRootPath}`)
        const functionProps = { 
        ...f,
        ...
        role: _props.role,
        securityGroups: f.securityGroupIds.map((secgroupId) => SecurityGroup.fromSecurityGroupId(this, `${id}-${secgroupId}-sg`, secgroupId)),
      };
      const lambdaFunction = new lambda.Function(
        this, `${id}-${functionProps.functionName}`,
        functionProps
      );
      if(f.eventProperties?.kinesisEventSource) {
        logger.info(`attaching data stream ${f.eventProperties?.kinesisEventSource.dataStreamArn}`)
         const stream = Stream.fromStreamArn(this, 'dataStreamSource', f.eventProperties?.kinesisEventSource.dataStreamArn)
         lambdaFunction.addEventSource(new lambdaEvent.KinesisEventSource(stream, {startingPosition: lambda.StartingPosition.LATEST}));
      }
      if(f.eventProperties?.sqsEventSource) {
        logger.info(`attaching sqs ${f.eventProperties?.sqsEventSource.queue.queueName}`)
        lambdaFunction.addEventSource(new lambdaEvent.SqsEventSource(f.eventProperties?.sqsEventSource.queue, f.eventProperties?.sqsEventSource.properties))
      }
