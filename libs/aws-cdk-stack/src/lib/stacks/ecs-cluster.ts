...
export class ECSCluster extends Stack {
  ...
  constructor(scope: Construct, id: string, stackProps: ECSClusterProps) {
    ...
    logger.info(`creating the ecs cluster with existing set as : ${stackProps.ecs.existingCluster}`);

    if (stackProps.ecs.existingCluster) {
      this.cluster = Cluster.fromClusterAttributes(this, stackProps.ecs.clusterName, {
        vpc: stackProps.vpc,
