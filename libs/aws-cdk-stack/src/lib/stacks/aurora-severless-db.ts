...
export class AuroraServerlessDbStack extends cdk.Stack {
  ...
  constructor(scope: cdk.Construct, id: string, _props: AuroraServerlessDbStackProperties) {
    super(scope, id, _props);
    this.templateOptions.description =
      "To produce sls (serverless) aurora";
    ...
    const db = new rds.ServerlessCluster(this, `${id}-aurora-serverless-cdk`, {
      ...
      securityGroups: _props.securityGroupIds.map((secgroupId) => SecurityGroup.fromSecurityGroupId(this, `${id}-${secgroupId}-sg`, secgroupId)),
      deletionProtection: _props.isProduction,
      enableDataApi: true,
      credentials: rds.Credentials.fromPassword(_props.username, cdk.SecretValue.plainText(_props.password))
    });
