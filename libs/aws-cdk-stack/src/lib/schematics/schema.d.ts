...
export function iscdkSchematicFlag(flag: cdkSchematicFlags | string): flag is cdkSchematicFlags {
  return cdkSchematicFlagsArray.includes(flag as cdkSchematicFlags);
}

export type Schema = { vpcPrivateSubnetIds: []
  subnetIds: string[]
  ...
  timeouts: number[]
  memmorySizes: number[] } & { [key in cdkSchematicFlags]?: string }
