import { convertNxGenerator, formatFiles, generateFiles, logger, names, offsetFromRoot, Tree } from "@nrwl/devkit";
...
import { IApplicationStackEnvironmentConfig, ServerlessApplicationStackConfiguration, SqsConfigurationBuilderOption, LambdaConfigurationBuilderOption } from "../../types";
import { SqsStackBuilder } from "../../stacks/builders/sqs-stack-builder";

function initStack(host: Tree, options: NormalizedSchema, filesRelativePath: string) {
  ...
  stagingEnvironmentConfig.sqs = [new SqsStackBuilder(stagingEnvironmentConfig, sqsOptions).BuildSqsStack()]
  ...
  for(let i = 0; i < options.subnetIds.length; i++)
  {
    for(let j = 0; j < options.availabilityZones.length; j++) {
      subnets.push({ id: options.subnetIds[i], availabilityZone: `${options.region}${options.availabilityZones[j]}` })
    }
  }
